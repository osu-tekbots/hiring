...
class ActionHandler {
     ...
     * 
     * Calls `checkRoleForPosition()` in authenticate.php to ensure the user has proper access for their desired action
     * 
     * @param string|string[] $allowedRoles The role(s) that should be allowed to access the current API call
     * @param string $positionID The ID of the position that the current API call is modifying
     * 
     * @return true|void
     */
    public function verifyUserRole($allowedRoles, $positionID) {
        if(!checkRoleForPosition($allowedRoles, $positionID)) {
     ...
     * 
     * @return mixed[] The decoded query string
     */ 
